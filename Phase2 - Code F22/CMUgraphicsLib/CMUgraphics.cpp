...
void window::DrawHexagon(const int iX1, const int iY1, const int iX2, const int iY2,
	const int iX3, const int iY3, const int iX4, const int iY4, const int iX5, const int
	iY5, const int iX6, const int iY6, const drawstyle dsStyle) {
	HRGN rgnTemp;
	POINT ptVertices[6] = { {iX1, iY1}, {iX2, iY2}, {iX3, iY3}, {iX4, iY4}, {iX5,
	iY5}, {iX6, iY6} };
	ProcessMessage(); // Kludge
	switch (dsStyle) {
	case FILLED:
		ChangeDrawStyle(FILLED);
		if (Polygon(dcActive, ptVertices, 6) != TRUE) {
	...
	case FRAME:
		...
		break;
	case INVERTED:
		rgnTemp = CreatePolygonRgn(ptVertices, 6, WINDING);
